...
class SendUtil(coinReader: CoinReader, coinWriter: CoinWriter, coinUtil: CoinUtil) {
  ...
  def sendCoinsWithKey(key: CoinPrivateKey,
    ...
    try {
      ...
      val recipientAddressesAmountsCopyFrom: Array[(String, String, Long, String, Option[InputBox])] = recipientAddressesAmounts.map {
        case (address, amount, tokenAmount, tokenId) => (address, amount, tokenAmount, tokenId, noInput)
      }

      val (amounts, tokens) = recipientAddressesAmountsCopyFrom.map {
        case (_, amount, tokenAmount, tokenId, _) => (BigInt(amount), (tokenId, BigInt(tokenAmount)))
      }.unzip
      ...
      copyBoxRegistersTo.foreach {
        case (additionalInput, outputIndex) if outputIndex < recipientAddressesAmountsCopyFrom.size =>
          val (address, amount, token, tokenId, _) = recipientAddressesAmountsCopyFrom(outputIndex)
          if (address != additionalInput.address) throw new Exception(s"Address of output ${outputIndex} does not match additional input address")
          recipientAddressesAmountsCopyFrom(outputIndex) = (address, amount, token, tokenId, Some(additionalInput))
        case (_, outputIndex) => throw new Exception(s"No output at index $outputIndex for copying additional input")
      }

      val selected = if (optimizeInputs) selectInputs(inputs, additionalInput, amounts.sum + txFeeLong, tokens) else inputs
      ...
      val outputBoxes: Array[OutputBox] = recipientAddressesAmountsCopyFrom.map {
        case (address, amount, tokenAmount, tokenId, copyFrom) =>
          ...
          coinUtil.getOutputBox(address, amount.toLong, allInputs, tokenAmount, tokenId, copyFrom)
      }
  ...
  private def selectInputs(inputBoxes: Array[InputBox], additionalInput: Option[InputBox], amountTotal: BigInt, tokens: Seq[(String, BigInt)]): Array[InputBox] = {
    // logic is as follows
