package org.ErgUrlWallet.db

import java.sql.{Connection, PreparedStatement, ResultSet, Statement}

import org.ErgUrlWallet.db.config.DBConfig
import org.ErgUrlWallet.db.core.DataStructures.{Col, Where, _}
import org.ErgUrlWallet.db.core.Table
import org.ErgUrlWallet.db.core.Util._
...
private [db] abstract class DBManagerDML(table:Table, dbConfig:DBConfig) {
  ...
  private [db] def insertArray(data:Array[Any])(implicit optConn:OptConn = None) = {
    ...
    if(data.length != actualDataCols.length) throw new Exception(s"Schema mismatch for table $table. Expected: ${actualDataCols.length}, found: ${data.length}. [AutoIncrment columns should not have data]")
