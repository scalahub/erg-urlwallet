...
package object ErgUrlWallet {
  ...
  case class ErgInputBox(id:String, amount:ScalaBigInt, registers:Map[String, String], ergoTreeHex:String, tokens:Seq[ErgToken], creationHeight:Int, address:String, spendingTxId:Option[String]) extends InputBox {
    val isSpent = spendingTxId.isDefined
    def getRegistersAsSeq:Seq[ErgoValue[_]] = registers.toSeq.sortBy(_._1).map(_._2).map{
      hex => deserialize(hex)
    }
  }
